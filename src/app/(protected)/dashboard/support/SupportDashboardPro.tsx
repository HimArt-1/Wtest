...
function getStatusInfo(status: string) {
    switch (status) {
        ...
        case "closed": return { label: "مغلقة", icon: X, color: "text-fg/40", bg: "bg-white/[0.03] border-white/[0.06]", dot: "bg-fg/30" };
        default: return { label: status, icon: MessageSquare, color: "text-fg/60", bg: "bg-white/5 border-white/10", dot: "bg-fg/30" };
    }
}
...
function getPriorityInfo(prio: string) {
    switch (prio) {
        ...
        case "normal": return { label: "عادية", color: "text-fg/50", bg: "bg-white/5", icon: Zap };
        case "low": return { label: "منخفضة", color: "text-fg/30", bg: "bg-white/[0.02]", icon: Clock };
        default: return { label: prio, color: "text-fg/50", bg: "bg-white/5", icon: Zap };
    }
}
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                {[
                    { label: "إجمالي التذاكر", value: stats.total, icon: Headphones, color: "text-fg/60", accent: "bg-white/[0.03] border-white/[0.06]" },
                    { label: "جديدة", value: stats.open, icon: MessageSquare, color: "text-blue-400", accent: stats.open > 0 ? "bg-blue-500/5 border-blue-500/20" : "bg-white/[0.03] border-white/[0.06]" },
                    { label: "قيد المعالجة", value: stats.inProgress, icon: Clock, color: "text-amber-400", accent: stats.inProgress > 0 ? "bg-amber-500/5 border-amber-500/20" : "bg-white/[0.03] border-white/[0.06]" },
                    { label: "عاجلة مفتوحة", value: stats.highPriority, icon: AlertTriangle, color: "text-red-400", accent: stats.highPriority > 0 ? "bg-red-500/5 border-red-500/20" : "bg-white/[0.03] border-white/[0.06]" },
            ...
            <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {[
                        ...
                        <button key={f.id} onClick={() => setFilter(f.id)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filter === f.id
                                ...
                                : "bg-white/[0.03] text-fg/40 hover:text-fg/60 hover:bg-white/[0.06] border border-white/[0.04]"}`}>
                            {f.label}
                        </button>
                    ))}
                </div>
                ...
                <button onClick={() => setSortBy(sortBy === "date" ? "priority" : "date")}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.03] border border-white/[0.06] rounded-lg text-xs text-fg/50 hover:text-fg/80 hover:bg-white/[0.06] transition-all">
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-surface/50 backdrop-blur-sm overflow-hidden">
                {filteredTickets.length === 0 ? (
                    <div className="p-16 text-center text-fg/20">
                        ...
                        <p className="text-xs text-fg/15 mt-1">جميع الأمور تسير على ما يرام!</p>
                    </div>
                ) : (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-0.5">
                                                <h4 className="text-sm font-bold text-fg/80 group-hover:text-gold transition-colors truncate">
                                                    {ticket.subject}
                                                </h4>
                                            </div>
                                            <div className="flex items-center gap-2 text-[11px]">
                                                <span className="text-fg/40">{userName}</span>
                                                <span className="text-fg/15">•</span>
                                                <span className="text-fg/30">{formatDistanceToNow(new Date(ticket.updated_at), { addSuffix: true, locale: ar })}</span>
                                            </div>
                                        </div>
            ...
            {/* ─── Footer Stats ─── */}
            <div className="flex items-center justify-between text-[11px] text-fg/20 px-1">
