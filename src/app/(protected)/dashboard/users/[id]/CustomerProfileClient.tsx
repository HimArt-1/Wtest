...
function getRoleBadge(role: string) {
    switch (role) {
        ...
        default: return { label: role, color: "text-fg/50 bg-white/5 border-white/10" };
    }
}
...
function getTicketStatusInfo(status: string) {
    switch (status) {
        ...
        case "closed": return { label: "مغلقة", color: "text-fg/30" };
        default: return { label: status, color: "text-fg/50" };
    }
}
    ...
    return (
        <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                className="rounded-2xl border border-white/[0.06] bg-surface/50 backdrop-blur-sm p-6">
                <div className="flex items-start gap-5">
                    {/* Avatar */}
                    <div className="w-20 h-20 rounded-2xl border-2 border-gold/20 bg-white/5 overflow-hidden shrink-0 flex items-center justify-center">
                        {avatarUrl ? (
                            <Image src={avatarUrl} alt="" width={80} height={80} className="object-cover w-full h-full" />
                        ) : (
                            <User className="w-8 h-8 text-fg/20" />
                        )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                            <h2 className="text-xl font-black text-fg">{String(profile.display_name || "مستخدم")}</h2>
                        ...
                        <p className="text-sm text-fg/40">@{String(profile.username || "—")}</p>
                        {profile.bio ? <p className="text-xs text-fg/30 mt-2 line-clamp-2">{String(profile.bio)}</p> : null}

                        <div className="flex items-center gap-4 mt-3 flex-wrap">
                            <span className="text-[11px] text-fg/25 flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    { label: "تذاكر مفتوحة", value: stats.openTickets, icon: Ticket, color: stats.openTickets > 0 ? "text-amber-400" : "text-fg/30", bg: stats.openTickets > 0 ? "bg-amber-500/5 border-amber-500/20" : "bg-white/[0.02] border-white/[0.06]" },
                ].map((s, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                    className="lg:col-span-2 rounded-2xl border border-white/[0.06] bg-surface/50 backdrop-blur-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-white/[0.04] flex items-center justify-between">
                        <h3 className="text-sm font-bold text-fg/80 flex items-center gap-2">
                            <ShoppingCart className="w-4 h-4 text-gold" /> سجل الطلبات
                        </h3>
                        <span className="text-[10px] text-fg/20">{orders.length} طلب</span>
                    </div>
                    {orders.length > 0 ? (
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-white/[0.04]">
                                        <th className="text-right px-5 py-2.5 text-fg/25 font-medium text-[11px]">رقم الطلب</th>
                                        <th className="text-right px-4 py-2.5 text-fg/25 font-medium text-[11px]">المبلغ</th>
                                        <th className="text-right px-4 py-2.5 text-fg/25 font-medium text-[11px]">الحالة</th>
                                        <th className="text-right px-4 py-2.5 text-fg/25 font-medium text-[11px]">الدفع</th>
                                        <th className="text-right px-4 py-2.5 text-fg/25 font-medium text-[11px]">العناصر</th>
                                        <th className="text-right px-5 py-2.5 text-fg/25 font-medium text-[11px]">التاريخ</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {orders.map((order: any) => (
                                        <tr key={order.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-fg/40 text-xs">{order.order_items?.length || 0} عنصر</td>
                        ...
                        <div className="p-12 text-center text-fg/15">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                    className="rounded-2xl border border-white/[0.06] bg-surface/50 backdrop-blur-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-white/[0.04] flex items-center justify-between">
                        <h3 className="text-sm font-bold text-fg/80 flex items-center gap-2">
                            <MessageSquare className="w-4 h-4 text-gold" /> تذاكر الدعم
                        </h3>
                        <span className="text-[10px] text-fg/20">{tickets.length} تذكرة</span>
                    </div>
                    {tickets.length > 0 ? (
                                    ...
                                    <Link key={t.id} href={`/dashboard/support/${t.id}`}>
                                        <div className="px-5 py-3 hover:bg-white/[0.02] transition-colors">
                                            <div className="flex items-center justify-between mb-1">
                                                <p className="text-xs font-bold text-fg/70 truncate flex-1">{t.subject}</p>
                        ...
                        <div className="p-10 text-center text-fg/15">
